...
# produces one line: 
#   $VAR1 = bless( do{\(my $o = 43254240)}, 'XML::LibXML::Document' );
...
eval {
   #$svg = $parser->parse_string( $xml );
   ##$parser->push( $xml );
   ##$svg = $parser->finish_push();
   ##$svg = XML::LibXML->load_xml(string => $xml);
    $twig->parse( $xml );
};
if ( $@ ) {
    print STDERR "error reported: $@\n";
    exit(1);
}
# $twig->print();
